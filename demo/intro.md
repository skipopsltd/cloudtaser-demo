# CloudCondom / Phantom — Live Demo

CloudCondom enables EU companies to use US cloud providers (AWS, GCP, Azure) with cryptographic guarantees that neither the provider nor US government can access your data.

## What you'll see

In this 10-minute demo, you'll:

1. **Deploy** a PostgreSQL pod with CloudCondom protection
...
4. **Witness** CloudCondom blocking an attempt to read `/proc/pid/environ`
5. **Review** the audit trail of the blocked access attempt

## How it works

CloudCondom's mutating webhook intercepts pod creation and injects a thin wrapper that:
...
- CloudCondom operator, wrapper, and eBPF agent pre-installed via Helm
